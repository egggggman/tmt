...
def _manifest() -> dict[str, object]:
    return {
        ...
        "decks": [
            {
                "cards": [
                    {"fragments": [{"semantic_key": "known-key", "limitations": ["unsupported"]}]}
                ]
            }
        ],
    }
...
def test_executed_class_requires_existing_action_evidence():
    snapshot = _snapshot()
    snapshot["conformance"]["executed_references"] = [
        {
            "semantic_key": "known-key",
            "evidence_kind": "bounded-action",
            "evidence_id": "e-1",
            "source_id": "object-1",
        }
    ]
...
def test_parameterized_runner_preserves_acceptance_001_gameplay():
    from scripts.run_acceptance_match_001 import run as run_001
    ...
    assert presence
    assert generic == run_001(ROOT, 7001)
